...
// background/browser/dom/service.ts

export class DOMService {
  /**
   * Calls buildDomTree in the active tab to retrieve a full DOM map.
   */
  static async getDomTree(tabId: number): Promise<any> {
      ...
      chrome.scripting.executeScript(
        {
          ...
          func: () => {
            // @ts-ignore
            return window.buildDomTree({ showHighlightElements: true });
          },
        },
        (results) => {
  ...
  /**
   * Finds a field in the DOM map by matching label/placeholder/name attributes.
   */
  static findFieldByLabel(domMap: Record<string, any>, label: string) {
    const matches = Object.values(domMap).filter((node: any) => {
      if (!node.attributes) return false;
      const attrs = node.attributes;
      return (
        (attrs.placeholder && attrs.placeholder.toLowerCase().includes(label.toLowerCase())) ||
        (attrs["aria-label"] && attrs["aria-label"].toLowerCase().includes(label.toLowerCase())) ||
        (attrs.name && attrs.name.toLowerCase().includes(label.toLowerCase()))
      );
    });
