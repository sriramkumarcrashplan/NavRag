...
export class NavigatorAgent extends BaseAgent<z.ZodType, NavigatorResult> {
  ...
  async fillForm(tabId: number, requiredFields: string[]) {
    const domTree: any = await DOMService.getDomTree(tabId);

    for (const field of requiredFields) {
      let node = DOMService.findFieldByLabel(domTree.map, field);

      // If CV couldn’t find it → ask user which highlighted box to use
      if (!node) {
        const choice = await Interactivity.askUser(
          `I couldn’t find a field for "${field}". Please tell me the box number (highlighted on screen).`,
          field
        );
        node = domTree.map[choice];
      }

      // Ask for value
      const value = await Interactivity.askUser(`Enter ${field}:`, field);

      // Use executor to input value
      await Executor.run("input", { tabId, xpath: node.xpath, value });
    }
  }
  /**
