...
const SidePanel = () => {
  ...
  const [mode, setMode] = useState<'automation' | 'rag'>('automation');
  ...
  const sendLegacyMessage = () => {
    ...
    // If answering a pending agent question
    if (pendingId) {
      ...
      // Free user message → send to background agent as a new task
      chrome.runtime.sendMessage({
        type: "user-task",
        text: input,
      });
  ...
  (async () => {
    ...
    try {
      ...
      if (!sid) {
        sid = await ragNewSession();
        await chrome.storage.local.set({ rag_session_id: sid });
      }
      setRagSessionId(sid);
      setRagHealthy(true);
  ...
  appendMessage({ actor: Actors.USER, content: q, timestamp: Date.now() });
